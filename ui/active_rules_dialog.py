...
class ActiveRulesDialog(ctk.CTkToplevel):
    ...
    def generate_rule_name(self, rule, lang):
        ...
        if "titan_hp_below" in cond:
            for t, val in cond["titan_hp_below"].items(): return f"{get_text(lang, 'ar_if_hp')} {get_text(lang, f'titan_{t}')} < {val}%"
        if "titan_energy_below" in cond:
            for t, val in cond["titan_energy_below"].items(): return f"{get_text(lang, 'ar_if_energy')} {get_text(lang, f'titan_{t}')} < {val}%"
        if "enemies_contain" in cond:
            enemies_str = ", ".join([get_text(lang, f"titan_{e}") for e in cond['enemies_contain']])
            return get_text(lang, "ar_enemies").format(enemies=enemies_str)
        return get_text(lang, "ar_no_name")
            ...
            if os.path.exists(path):
                with open(path, 'r', encoding='utf-8') as f:
                    ...
                    for i, rule in enumerate(rules):
                        ...
                        team = rule.get("team", [])
                        action_text = get_text(lang, "ar_stop") if team == ["STOP"] else get_text(lang, "ar_change").format(count=len(team))
    ...
    def edit_rule(self, room_type, rule_index, rule_data):
        def on_rule_saved(rule_name, r_type):
            self.refresh_rules_list()
        RuleBuilderDialog(self.controller, room_type, on_rule_saved, edit_index=rule_index, edit_data=rule_data)
