...
class RuleBuilderDialog(ctk.CTkToplevel):
    def __init__(self, master, room_type, callback, edit_index=None, edit_data=None):
        ...
        self.title(get_text(lang, title_key).format(room=get_text(lang, f"elem_{room_type}")))
        ...
        self.lbl_title = ctk.CTkLabel(self, text=get_text(lang, "rb_title").format(room=get_text(lang, f"elem_{room_type}")), font=ctk.CTkFont(size=18, weight="bold"))
        self.lbl_title.grid(row=0, column=0, pady=(15, 10))

        self.entry_name = ctk.CTkEntry(self, placeholder_text=get_text(lang, "rb_name"), width=480)
        self.entry_name.grid(row=1, column=0, pady=5)

        self.segment_type = ctk.CTkSegmentedButton(
            self, 
            values=[get_text(lang, "rb_type_hp"), get_text(lang, "rb_type_energy"), get_text(lang, "rb_type_enemies")],
        ...
        display_titans = [get_text(lang, f"titan_{t}") for t in self.available_titans_en]
        ...
        ctk.CTkLabel(self.hp_frame, text=get_text(lang, "rb_hp_below")).grid(row=0, column=1, padx=5, pady=10)
        ...
        ctk.CTkLabel(self.energy_frame, text=get_text(lang, "rb_energy_below")).grid(row=0, column=1, padx=5, pady=10)
        ...
        ctk.CTkLabel(self.enemies_frame, text=get_text(lang, "rb_enemies_pick")).grid(row=0, column=0, columnspan=3, pady=(5, 5))
        
        row, col = 1, 0
        for enemy_en in DUNGEON_ENEMIES_EN:
            var = ctk.BooleanVar(value=False)
            self.enemy_vars[enemy_en] = var
            chk = ctk.CTkCheckBox(self.enemies_frame, text=get_text(lang, f"titan_{enemy_en}"), variable=var)
        ...
        self.chk_stop = ctk.CTkCheckBox(self.action_frame, text=get_text(lang, "rb_action_stop"), variable=self.action_is_stop, command=self.toggle_action)
        self.chk_stop.pack(pady=(0, 10))

        self.btn_pick_team = ctk.CTkButton(self.action_frame, text=get_text(lang, "rb_btn_pick"), fg_color="#007bff", hover_color="#0056b3", command=self.pick_team)
        self.btn_pick_team.pack()
        
        self.lbl_team_preview = ctk.CTkLabel(self.action_frame, text=get_text(lang, "rb_team_preview_none"), text_color="yellow")
        self.lbl_team_preview.pack()

        self.btn_save = ctk.CTkButton(self, text=get_text(lang, "rb_btn_save"), fg_color="#28a745", hover_color="#218838", state="disabled", command=self.save_rule)
        self.btn_save.grid(row=5, column=0, pady=20, padx=60, sticky="ew")

        if edit_data:
            self.load_edit_data(edit_data, lang)
        else:
            self.segment_type.set(get_text(lang, "rb_type_hp"))
            self.on_type_change(get_text(lang, "rb_type_hp"))
            self.entry_hp_val.insert(0, "35")
            self.entry_en_val.insert(0, "100")

    def load_edit_data(self, data, lang):
        ...
        if "titan_hp_below" in cond:
            ...
            for t, val in cond["titan_hp_below"].items():
                self.opt_titan_hp.set(get_text(lang, f"titan_{t}"))
                self.entry_hp_val.insert(0, str(val))
        elif "titan_energy_below" in cond:
            self.segment_type.set(get_text(lang, "rb_type_energy"))
            self.on_type_change(get_text(lang, "rb_type_energy"))
            for t, val in cond["titan_energy_below"].items():
                self.opt_titan_en.set(get_text(lang, f"titan_{t}"))
                self.entry_en_val.insert(0, str(val))
        elif "enemies_contain" in cond:
            self.segment_type.set(get_text(lang, "rb_type_enemies"))
            self.on_type_change(get_text(lang, "rb_type_enemies"))
        ...
        if team == ["STOP"]:
            ...
            self.lbl_team_preview.configure(text=get_text(lang, "rb_team_preview").format(team=team_str), text_color="#28a745")
    ...
    def toggle_action(self):
        ...
        if self.action_is_stop.get():
            ...
            self.lbl_team_preview.configure(text=get_text(lang, "rb_action_stop_preview"), text_color="#dc3545")
            self.btn_save.configure(state="normal")
            self.target_team = ["STOP"]
        else:
            self.btn_pick_team.configure(state="normal")
            if not self.target_team or self.target_team == ["STOP"]:
                self.lbl_team_preview.configure(text=get_text(lang, "rb_team_preview_none"), text_color="yellow")
                self.btn_save.configure(state="disabled")
                self.target_team = []
            else:
                team_str = ", ".join([get_text(lang, f"titan_{t}") for t in self.target_team])
                self.lbl_team_preview.configure(text=get_text(lang, "rb_team_preview").format(team=team_str), text_color="#28a745")
                self.btn_save.configure(state="normal")
    ...
    def on_type_change(self, value):
        ...
        if value == get_text(lang, "rb_type_hp"): self.hp_frame.grid(row=3, column=0, padx=40, pady=10, sticky="ew")
        elif value == get_text(lang, "rb_type_energy"): self.energy_frame.grid(row=3, column=0, padx=40, pady=10, sticky="ew")
        elif value == get_text(lang, "rb_type_enemies"): self.enemies_frame.grid(row=3, column=0, padx=40, pady=10, sticky="ew")
    ...
    def pick_team(self):
        def on_selected(team):
            ...
            self.lbl_team_preview.configure(text=get_text(lang, "rb_team_preview").format(team=team_str), text_color="#28a745")
            self.btn_save.configure(state="normal")
        TeamSelectorDialog(self.master, on_selected, room_type=self.room_type, context="rule")
    ...
    def save_rule(self):
        ...
        def get_en_key(disp_text):
            for t in self.available_titans_en:
                if get_text(lang, f"titan_{t}") == disp_text: return t
            return self.available_titans_en[0]

        if selected_type == get_text(lang, "rb_type_hp"):
            t_en = get_en_key(self.opt_titan_hp.get())
            condition = {"titan_hp_below": {t_en: int(self.entry_hp_val.get() or 35)}}
        elif selected_type == get_text(lang, "rb_type_energy"):
            t_en = get_en_key(self.opt_titan_en.get())
