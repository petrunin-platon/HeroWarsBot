...
def get_rois(window_rect):
    """Разбивка экрана на верхнюю (выбор) и нижнюю (текущий пак) зоны"""
...
def verify_and_set_team(target_pack, available_titans, window_rect, sct):
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        # Возврат в начало сетки двумя свайпами
        swipe_scrcpy(window_rect, direction="up") 
        time.sleep(0.1)
        swipe_scrcpy(window_rect, direction="up") 
        time.sleep(0.3)
        
        # 1. ЧИСТАЯ МЕХАНИКА ОЧИСТКИ (С учетом сдвига иконок влево)
            ...
            for titan in GAME_ORDER:
                if titan not in sorted_pack:
                    ...
                    if coords:
                        ...
                        time.sleep(0.4) # ЖДЕМ 0.4 сек пока титаны сдвинутся влево!
                        found_unwanted = True
                        break # Обрываем цикл for, чтобы отсканировать сдвинутые слоты заново
            
            if not found_unwanted:
                break # Лишних титанов в слотах больше нет
            cleanup_attempts += 1
        ...
        for titan in sorted_pack:
            ...
            for swipe_idx in range(3): 
                ...
                if swipe_idx < 2:
                    ...
                    time.sleep(0.3) # Ждем прогрузки интерфейса после свайпа
                    
            if not found:
                print(f"[ОШИБКА] Не нашел {titan} даже после 2 свайпов!")

        # 3. КОНТРОЛЬНАЯ ПРОВЕРКА (Синхронизация с анимацией полета иконки)
        print("[КОМАНДА] Выполняю контрольную проверку состава...")
        time.sleep(1.2) # ФИКС: Даем последнему выбранному титану анимационно "долететь" до слота!
        all_perfect = True
        ...
        if all_perfect:
            ...
            print(f"[ПРЕДУПРЕЖДЕНИЕ] Сборка не прошла проверку. Перехожу к следующей попытке.")
